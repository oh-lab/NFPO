...
def _parse_master_port_range(master_port_range: Optional[list[int]] = None) -> tuple[int, int]:
    if master_port_range is not None:
        return int(master_port_range[0]), int(master_port_range[1])

    range_str = os.getenv("VERL_MASTER_PORT_RANGE", "15000:25000")
    separators = (":", "-", ",")
    for separator in separators:
        ...
        raise ValueError(
            "VERL_MASTER_PORT_RANGE must use one of ':', '-' or ',' as a separator, "
            f"got {range_str!r}."
        )

    if start <= 0 or end <= start:
        raise ValueError(f"Invalid VERL_MASTER_PORT_RANGE: {range_str!r}")
    return start, end


def _iter_master_ports(start: int, end: int, last_port: int):
    next_port = last_port + 1 if start <= last_port < end else start
    yield from range(next_port, end)
    yield from range(start, next_port)


def _allocate_master_port(addr: str, master_port_range: Optional[list[int]] = None) -> int:
    start, end = _parse_master_port_range(master_port_range)
    state_file = os.getenv("VERL_MASTER_PORT_STATE_FILE", f"/tmp/verl-master-port-{os.getuid()}.txt")
    ...
    with open(state_file, "a+", encoding="utf-8") as f:
        ...
        for port in _iter_master_ports(start, end, last_port):
    ...
    raise RuntimeError(f"Could not find a free port in range [{start}, {end}) for addr={addr}")
...
def get_master_addr_port(master_port_range: Optional[list[int]] = None) -> tuple[str, str]:
    ...
    port = _allocate_master_port(addr, master_port_range=master_port_range)
