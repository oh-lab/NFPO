...
class RolloutCorrectionConfig(BaseConfig):
    ...
    Args:
        ...
        loss_type (str): Loss function type in bypass mode (bypass_mode=True).
            ...
            - "oapl": Optimal Advantage Policy Learning regression loss using
              sequence-level log-ratio against rollout log-probabilities
            Default: "ppo_clip"

        oapl_log_ratio_beta (float): Beta that scales the sequence log-ratio term in the
            OAPL loss, i.e. beta * log(π_theta / π_behavior). Default: 1e-3

        oapl_vhat_beta (float): Beta used for the group-wise soft value baseline
            V_hat = beta * log((1/G) * Σ exp(r / beta)). Default: 1.0
    ...
    oapl_log_ratio_beta: float = 1e-3
    oapl_vhat_beta: float = 1.0
