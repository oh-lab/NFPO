...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        if is_muon_optimizer(self.config.optim):
            effective_fsdp_size = get_effective_fsdp_size(self.device_mesh.size(), self.config.model.fsdp_config.fsdp_size)
            if effective_fsdp_size != 1:
                raise ValueError(
                    "MuonWithAuxAdam requires replicated parameters in VERL SFT. "
                    "Set `model.fsdp_config.fsdp_size=1`."
                )
            if fsdp_strategy != "fsdp":
                raise ValueError("MuonWithAuxAdam is only supported with `model.strategy=fsdp` in this trainer.")
