...
def count_validation_uids_by_source(data_sources: list[str], sample_uids: list[str]) -> dict[str, int]:
    """Count unique validation prompts for each data source."""
    data_src2uids = defaultdict(set)
    for data_source, sample_uid in zip(data_sources, sample_uids, strict=True):
        data_src2uids[str(data_source)].add(str(sample_uid))
    return {data_source: len(uids) for data_source, uids in data_src2uids.items()}


def aggregate_validation_core_metrics(
    data_src2var2metric2val: dict[str, dict[str, dict[str, float]]],
    data_src2num_prompts: dict[str, int],
) -> dict[str, float]:
    """
    Aggregate dataset-level core validation metrics across all data sources.

    Returns overall macro and micro averages with keys like:
      val-core/all/reward/mean@32/macro_avg
      val-core/all/reward/mean@32/micro_avg
    """
    aggregate_store = defaultdict(lambda: {"values": [], "weights": []})

    for data_source, var2metric2val in data_src2var2metric2val.items():
        prompt_count = data_src2num_prompts.get(str(data_source), 0)
        if prompt_count <= 0:
            continue
        ...
        for var_name, metric2val in var2metric2val.items():
            ...
            for metric_name, metric_val in metric2val.items():
                ...
                if not is_core_metric:
                    continue

                store_key = (var_name, metric_name)
                aggregate_store[store_key]["values"].append(float(metric_val))
                aggregate_store[store_key]["weights"].append(float(prompt_count))

    aggregated_metrics = {}
    for (var_name, metric_name), payload in aggregate_store.items():
        values = payload["values"]
        weights = payload["weights"]
        aggregated_metrics[f"val-core/all/{var_name}/{metric_name}/macro_avg"] = float(np.mean(values))
        aggregated_metrics[f"val-core/all/{var_name}/{metric_name}/micro_avg"] = float(
