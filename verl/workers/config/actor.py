...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Use "nfpo" or its alias "forward_trace".
        mask_delta (float): Trust-region delta for forward-trace token masking.
        forward_trace_mask_mode (str): Mask divergence mode.
        n_step_forward_trace (int): Number of future tokens included in the forward trace.
    """

    loss_mode: str = "nfpo"
    mask_delta: float = 0.2
    forward_trace_mask_mode: str = "forward_trace_binary_tv"
    n_step_forward_trace: int = 1
    forward_trace_ratio_clip: float = 3.0
    forward_trace_lower: float = 0.8
    forward_trace_upper: float = 1.2
    forward_trace_use_rollout_old_log_probs: bool = False
...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        clip_ratio_low (float): Lower bound for PPO clipping ratio and negative-side TV threshold.
        clip_ratio_high (float): Upper bound for PPO clipping ratio and positive-side TV threshold.
