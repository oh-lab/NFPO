...
def is_muon_optimizer(config_or_name) -> bool:
    optimizer_name = config_or_name.optimizer if hasattr(config_or_name, "optimizer") else str(config_or_name)
    return optimizer_name.lower() in {"muonwithauxadam", "singledevicemuonwithauxadam"}
...
def _build_muon_optimizer(config: FSDPOptimizerConfig, named_parameters):
    import importlib

    if named_parameters is None:
        raise ValueError(
            f"{config.optimizer_impl}.{config.optimizer} requires `named_parameters` so VERL can split Muon and "
            "auxiliary Adam parameter groups."
        )

    overrides = dict(config.override_optimizer_config or {})
    muon_lr = overrides.pop("muon_lr", config.lr)
    muon_momentum = overrides.pop("muon_momentum", 0.95)
    muon_weight_decay = overrides.pop("muon_weight_decay", config.weight_decay)
    aux_lr = overrides.pop("aux_lr", config.lr)
    aux_eps = overrides.pop("aux_eps", 1e-10)
    aux_weight_decay = overrides.pop("aux_weight_decay", config.weight_decay)
    aux_exclude = set(MUON_EXCLUDE_NAME_SUBSTRINGS)
    aux_exclude.update(_normalize_filter_values(overrides.pop("muon_exclude_names", None)))
    muon_include = set(_normalize_filter_values(overrides.pop("muon_include_names", None)))

    if "aux_betas" in overrides:
        aux_betas = tuple(overrides.pop("aux_betas"))
    else:
        aux_betas = (
            overrides.pop("aux_beta1", config.betas[0]),
            overrides.pop("aux_beta2", config.betas[1]),
        )

    if overrides:
        unsupported = ", ".join(sorted(overrides))
        raise ValueError(
            f"Unsupported Muon override_optimizer_config keys: {unsupported}. "
            "Use muon_lr/muon_momentum/muon_weight_decay/aux_lr/aux_beta1/aux_beta2/aux_eps/"
            "aux_weight_decay/muon_exclude_names/muon_include_names."
        )
    ...
    for name, param in named_parameters:
        ...
        force_muon = any(token in name_lower for token in muon_include)
        force_aux = any(token in name_lower for token in aux_exclude)
        if param.ndim >= 2 and (force_muon or not force_aux):
            muon_params.append(param)
            muon_numel += param.numel()
        else:
            aux_params.append(param)
            aux_numel += param.numel()

    if not muon_params:
        raise ValueError(
            f"{config.optimizer_impl}.{config.optimizer} did not find any Muon-compatible parameters. "
            "If you are using FSDP, keep the actor unsharded by setting `actor_rollout_ref.actor.fsdp_config.fsdp_size=1`."
        )
    ...
    print(
        f"Using {config.optimizer_impl}.{config.optimizer}: "
        f"muon_params={len(muon_params)} ({muon_numel:,} params), "
        f"aux_params={len(aux_params)} ({aux_numel:,} params)"
    )
...
def build_optimizer(parameters, config: FSDPOptimizerConfig, named_parameters=None):
    ...
    Args:
        ...
        named_parameters: Optional `(name, parameter)` iterable used by optimizers like MuonWithAuxAdam
            that need per-parameter routing.
    ...
    if is_muon_optimizer(config):
        return _build_muon_optimizer(config, named_parameters=named_parameters)
