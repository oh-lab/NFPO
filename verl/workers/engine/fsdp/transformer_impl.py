...
class FSDPEngine(BaseEngine):
    ...
    def _build_optimizer(self, module, named_parameters=None):
    ...
    def _build_model_optimizer(self):
        ...
        if is_muon_optimizer(self.optimizer_config):
            effective_fsdp_size = get_effective_fsdp_size(self.device_mesh.size(), self.engine_config.fsdp_size)
            if effective_fsdp_size != 1:
                raise ValueError(
                    "MuonWithAuxAdam requires replicated parameters in VERL FSDP engine. "
                    "Set `fsdp_config.fsdp_size=1`."
                )
            if self.engine_config.strategy != "fsdp":
                raise ValueError("MuonWithAuxAdam is only supported with `strategy=fsdp` in this engine.")
        ...
        if not self.engine_config.forward_only:
            ...
            optimizer = self._build_optimizer(module, named_parameters=named_parameters)
