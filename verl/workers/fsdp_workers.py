...
from verl.workers.config.optimizer import build_optimizer, is_muon_optimizer
...
def get_effective_fsdp_size(world_size, fsdp_size):
    return world_size if fsdp_size < 0 or fsdp_size >= world_size else fsdp_size
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if self._is_actor and is_muon_optimizer(self.config.actor.optim):
            effective_fsdp_size = get_effective_fsdp_size(world_size, self.config.actor.fsdp_config.get("fsdp_size", -1))
            if effective_fsdp_size != 1:
                raise ValueError(
                    "MuonWithAuxAdam requires replicated actor parameters in VERL. "
                    "Set `actor_rollout_ref.actor.fsdp_config.fsdp_size=1`."
                )
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None and is_muon_optimizer(optim_config):
            effective_fsdp_size = get_effective_fsdp_size(self.world_size, fsdp_config.get("fsdp_size", -1))
            if effective_fsdp_size != 1:
                raise ValueError(
                    "MuonWithAuxAdam requires replicated actor parameters in VERL. "
                    "Set `actor_rollout_ref.actor.fsdp_config.fsdp_size=1`."
            ...
            if fsdp_strategy != "fsdp":
                raise ValueError("MuonWithAuxAdam is only supported with `actor_rollout_ref.actor.strategy=fsdp`.")
            self.use_orig_params = True
        ...
        actor_named_parameters = list(actor_module.named_parameters())
        ...
        if role == "actor" and optim_config is not None:
            ...
            actor_optimizer = build_optimizer(
                ...
                optim_config,
                named_parameters=actor_named_parameters,
            )
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config: FSDPCriticConfig):
        ...
        critic_named_parameters = list(critic_module.named_parameters())
        ...
        self.use_orig_params = fsdp_config.get("use_orig_params", False)
        if is_muon_optimizer(config.optim):
            effective_fsdp_size = get_effective_fsdp_size(self.world_size, fsdp_config.get("fsdp_size", -1))
            if effective_fsdp_size != 1:
                raise ValueError(
                    "MuonWithAuxAdam requires replicated critic parameters in VERL. "
                    "Set `critic.model.fsdp_config.fsdp_size=1` if you want to use it for the critic."
                )
            if config.strategy != "fsdp":
                raise ValueError("MuonWithAuxAdam is only supported with `critic.strategy=fsdp`.")
            self.use_orig_params = True
        ...
        critic_optimizer = build_optimizer(
            ...
            config.optim,
            named_parameters=critic_named_parameters,
        )
