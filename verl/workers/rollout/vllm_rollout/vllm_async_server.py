...
from verl.utils.net_utils import get_exclusive_port, get_exclusive_port_block, is_valid_ipv6_address
...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        if self.node_rank == 0:
            ...
            control_port_range = os.getenv("VERL_VLLM_CONTROL_PORT_RANGE", "45000:47000")
            control_state_file = os.getenv(
                "VERL_VLLM_CONTROL_PORT_STATE_FILE", f"/tmp/verl-vllm-control-port-{os.getuid()}.txt"
            )
            vllm_block_size = int(os.getenv("VERL_VLLM_PORT_BLOCK_SIZE", "32"))
            vllm_block_range = os.getenv("VERL_VLLM_PORT_BLOCK_RANGE", "47000:55000")
            vllm_block_state_file = os.getenv(
                "VERL_VLLM_PORT_BLOCK_STATE_FILE", f"/tmp/verl-vllm-port-block-{os.getuid()}.txt"
            )
            # used for torch.distributed.init_process_group
            self._master_port, self._master_sock = get_exclusive_port(
                self._server_address, port_range=control_port_range, state_file=control_state_file
            )
            # used for data parallel: --data-parallel-address, --data-parallel-rpc-port
            self._dp_rpc_port, self._dp_rpc_sock = get_exclusive_port(
                self._server_address, port_range=control_port_range, state_file=control_state_file
            )
            self._dp_master_port, self._dp_master_sock = get_exclusive_port(
                self._server_address, port_range=control_port_range, state_file=control_state_file
            )
            # Reserve a contiguous block for vLLM internal multiprocessing.
            # vLLM may increment VLLM_PORT for additional internal sockets, so
            # a unique base port alone is insufficient when many replicas start
            # concurrently on the same node.
            self._vllm_port, self._vllm_socks = get_exclusive_port_block(
                self._server_address,
                block_size=vllm_block_size,
                port_range=vllm_block_range,
                state_file=vllm_block_state_file,
            )
        else:
    ...
    def _release_reserved_ports(self):
        """Release locally reserved ports right before vLLM binds them."""
        for attr in ("_master_sock", "_dp_rpc_sock", "_dp_master_sock"):
            sock = getattr(self, attr, None)
            if sock is not None:
                sock.close()
                setattr(self, attr, None)
        for sock in getattr(self, "_vllm_socks", []):
            sock.close()
        self._vllm_socks = []
    ...
    async def run_server(self, args: argparse.Namespace):
        if self._vllm_port is not None:
            # Give each rollout replica a deterministic local base port so
            # concurrent vLLM servers do not race on get_open_port().
            os.environ["VLLM_PORT"] = str(self._vllm_port)
